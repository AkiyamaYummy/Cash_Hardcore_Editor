...
function initEle(ele) {
    ...
    $(eventarea).mouseup(function (event) {
        editPageCheck(ele);
        if(event.which == 1)
            showNewContentPanel(ele,getMousePosi(event));
    });
    $(eventarea).bind("contextmenu", function (event) {
        editPageRightClick(ele, event);
        return false;
    });
    $(ele).append(eventarea);
}
function editPageRightClick(ele,e) {
    showEditPageRightClickPanel(ele,e);
}
function showEditPageRightClickPanel(ele,e) {
